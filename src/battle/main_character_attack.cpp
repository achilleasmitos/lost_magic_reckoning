...
void MainCharacterAttack(MainCharacter& main_character, Creature& target)
{
	...
	if (hit_main_character_roll >= target.get_ac())
	{
		...
		std::cout << "The " << target.get_display_name() << "'s HP is now "
				  << target.get_hp() << std::endl;
	}

	else
	{
		std::cout << "The " << target.get_display_name() << " has better AC than the attack of the main character, main character misses!"
				  << std::endl;
	}
}
